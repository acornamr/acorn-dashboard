output$bsi_summary <- renderText({
  ...
  ecoli_bsi <- acorn_dta_filter() %>% fun_filter_blood_only() %>% filter(bsi_pahtogen == "ECOLI") %>% pull(redcap_id) %>% unique()
  staph_bsi <- acorn_dta_filter() %>% fun_filter_blood_only() %>% filter(bsi_pahtogen == "STAPH") %>% pull(redcap_id) %>% unique()
