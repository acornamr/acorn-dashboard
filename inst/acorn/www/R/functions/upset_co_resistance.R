upset_co_resistance <- function(data_input, organism_input, corresp, deduplication_method) {
  ...
  ComplexUpset::upset(dta, colnames(dta)[-1], name = " ", width_ratio = 0.2, stripes = "white",  
                      ...
                      themes = ComplexUpset::upset_default_themes(text = element_text(size = 14), 
                      ...
                      set_sizes = ComplexUpset::upset_set_size(position = "right") +
                        ...
                        scale_y_continuous(breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1))))),
                      matrix = (
                        ComplexUpset::intersection_matrix(geom = geom_point(size = 4), 
                                                          segment = geom_segment(linetype = 'dotted')) +
                          scale_color_manual(values = c("TRUE" = cols_sir[3], "FALSE" = cols_sir[1])))
  )
}
